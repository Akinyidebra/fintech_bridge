...
class ProviderProfileIdentificationSection extends StatelessWidget {
  final Map<String, dynamic>? identificationImages;

  const ProviderProfileIdentificationSection({
    super.key,
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                Text(
                  'Upload your Business Registration Certificate, Tax PIN Certificate, and other business documents for verification',
                  style: TextStyle(
                    fontSize: 12,
  ...
  String _getDocumentLabel(String key) {
    // Map the Firestore keys to user-friendly labels for business documents
    switch (key.toLowerCase()) {
      case 'businessregistration':
      case 'businessregistrationfront':
        return 'Business Registration';
      case 'businessregistrationback':
        return 'Business Registration (Back)';
      case 'taxpincertificate':
      case 'taxpinfront':
        return 'Tax PIN Certificate';
      case 'taxpinback':
        return 'Tax PIN Certificate (Back)';
      case 'businesslicense':
      case 'businesslicensefront':
        return 'Business License';
      case 'businesslicenseback':
        return 'Business License (Back)';
  ...
  IconData _getDocumentIcon(String key) {
    // Determine icon based on document type for business documents
    final lowerKey = key.toLowerCase();
    
    if (lowerKey.contains('business') && lowerKey.contains('registration')) {
      return Icons.business_center_rounded; // Business Registration
    } else if (lowerKey.contains('tax') || lowerKey.contains('kra') || lowerKey.contains('pin')) {
      return Icons.receipt_long_rounded; // Tax/KRA documents
    } else if (lowerKey.contains('license') || lowerKey.contains('permit')) {
      return Icons.verified_rounded; // License/Permit
    } else if (lowerKey.contains('national')) {
      return Icons.credit_card_rounded; // National ID
    } else if (lowerKey.contains('incorporation') || lowerKey.contains('certificate')) {
      return Icons.workspace_premium_rounded; // Certificates
    } else if (lowerKey.contains('memorandum')) {
      return Icons.article_rounded; // Legal documents
    } else {
